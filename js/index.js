...
var colorOptions = [...colors, ...colors];
...
const buildMemory = (color) => {
  ...
  box.addEventListener("click", () => {
    var revealed = box.getAttribute("data-revealed");
    if (awaitingEndOfMove || revealed === "true" || activeTile === box) {
      return;
    }
    ...
    colorMatch = activeTile.getAttribute("data-color");
    ...
    if (colorMatch === color) {
      ...
      activeTile = null;
      revealedCount += 2;
      score.textContent = revealedCount.toString();
      if (revealedCount === colorsLength) {
        alert(
...
for (var i = 0; i < colorsLength; i++) {
  var colorIndex = Math.floor(Math.random() * colorOptions.length);
